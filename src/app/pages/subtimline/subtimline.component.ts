...
@Component({
  selector: 'app-subtimline',
  templateUrl: './subtimline.component.html',
  styleUrls: ['./subtimline.component.scss']
})
export class SubtimlineListComponent implements OnInit {
  topics2 = [];
  list = true;
  lastView = false;
  bookId: number;
  ...
  ngOnInit() {
    ...
    this.bookService.getSubTimelineById(id).subscribe(
      res => {
        console.log(res),
    ...
    if (confirm('Are you sure to delete this record ?')) {
      this.bookService.deleteTimelineId(+id)
        .subscribe(
          res => {
            console.log(res)
            this.bookService.getSubTimelineById(this.snapId).subscribe(
              data => {
                this.list = false;
